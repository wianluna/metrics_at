...
def _get_net(net: str, pretrained: bool) -> nn.modules.container.Sequential:
    ...
    if _TORCHVISION_GREATER_EQUAL_0_13:
        if pretrained:
            pretrained_features = getattr(tv, net)(
                weights=getattr(tv, _weight_map[net]).IMAGENET1K_V1
            ).features
        else:
...
class SqueezeNet(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> NamedTuple:
        """Process input."""
        squeeze_output = namedtuple(
            "squeeze_output", ["relu1", "relu2", "relu3", "relu4", "relu5", "relu6", "relu7"]
        )
...
class Vgg16(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> NamedTuple:
        ...
        vgg_outputs = namedtuple(
            "vgg_outputs", ["relu1_2", "relu2_2", "relu3_3", "relu4_3", "relu5_3"]
        )
...
class ScalingLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.register_buffer(
            "shift", torch.Tensor([-0.030, -0.088, -0.188])[None, :, None, None], persistent=False
        )
        self.register_buffer(
            "scale", torch.Tensor([0.458, 0.448, 0.450])[None, :, None, None], persistent=False
        )
...
def learned_perceptual_image_patch_similarity(
    ...
    """The Learned Perceptual Image Patch Similarity (`LPIPS_`) calculates the perceptual
    similarity between two images.

    LPIPS essentially computes the similarity between the activations of two image patches
    for some pre-defined network.
    This measure has been shown to match human perception well. A low LPIPS score means that image
    patches are
    perceptual similar.

    Both input image patches are expected to have shape ``(N, 3, H, W)``. The minimum size
    of `H,W` depends on the chosen backbone (see `net_type` arg).

    Args:
        img1: first set of images
        img2: second set of images
        net_type: str indicating backbone network type to use.
        Choose between `'alex'`, `'vgg'` or `'squeeze'`
        reduction: str indicating how to reduce over the batch dimension.
        Choose between `'sum'` or `'mean'`.
        normalize: by default this is ``False`` meaning that the input is expected
        to be in the [-1,1] range. If set to ``True`` will instead expect input
